...
class IntCodeComputer(val memory: IntArray, val ioInput: suspend (Int) -> Int, val ioOutput: suspend (Int) -> Unit) {
  enum class Instruction(
    val opCode: Int,
    val paramCount: Int,
    val fn: suspend (c: IntCodeComputer, params: IntArray, inputParams: IntArray) -> Boolean) {
    ...
    INPUT(3,
      1,
      { c, params, _ -> c.memory[params[0]] = c.ioInput(c.inputCounter++); false }
    ),
    OUTPUT(4,
      1,
  ...
  suspend fun runUntilHalt() {
    while(tick()) { /* do nothing */}
  }
}

suspend fun runProgram(memory: IntArray, ioInput: suspend (Int) -> Int, ioOutput: suspend (Int) -> Unit) {
  IntCodeComputer(memory, ioInput, ioOutput).runUntilHalt()
}
